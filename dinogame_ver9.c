...
void display(int x, int y, char* s) {
    GotoXY(x, y);
    printf("%s", s);
}
...
void DrawTree(int TREE_X) {
    ...
    // printf(" $$ ");
    display(TREE_X, TREE_BOTTOM_Y, "$$$$");
    for (int i = 1; i < 5; i++) {
        display(TREE_X, TREE_BOTTOM_Y + i, " $$ ");
    }
}
...
void DrawPyramid(int PYRAMID_X) {
    ...
    // printf("*******************");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y, "         *         ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 1, "        ***        ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 2, "       *****       ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 3, "      *******      ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 4, "     *********     ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 5, "    ***********    ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 6, "   *************   ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 7, "  ***************  ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 8, " ***************** ");
    display(PYRAMID_X, PYRAMID_BOTTOM_Y + 9, "*******************");
}
...
// 난수에 따라 장애물을 그리는 함수
void DrawObstacle(int obstaclecase, int obstaclex) {
    switch (obstaclecase) {
    case 1:
        DrawTree(obstaclex);
        break;
    case 2:
        DrawPyramid(obstaclex);
        break;
    default:
        break;
    }
...
bool isCollision(int obstaclecase, int obstaclex, int dinoy) {
    switch (obstaclecase) {
    case 1:
        return TreeCollision(obstaclex, dinoy);
        break;
    case 2:
        return PyramidCollision(obstaclex, dinoy);
    default:
        return -1;
        break;
    }
...
// 공룡의 움직임을 계산하는 함순
void DinoMove(bool isJumping, int* pdinoy, int dinodelta) {
    if (isJumping) {
        *pdinoy -= dinodelta;
    } else {
        *pdinoy += dinodelta;
...
int main(void) {
    ...
    Delta Pyramid = { 4, 6 };
    while (true) {
        int dinodelta = DinoDelta(obstaclecase, Tree, Pyramid);
        int dinoy = DINOSAUR_BOTTOM_Y;
