...
bool isCollision(const int obstaclecase, int obstaclex, int dinoy) {
    switch (obstaclecase) {
    case 1:
...
int ObstacleDelta(const int obstaclecase, Delta d1, Delta d2) {
    switch (obstaclecase) {
    case 1:
        return d1.obstacleh;
        break;
...
// 공룡의 움직임을 계산하는 함순
void DinoMove(bool* pisJumping, int* pdinoy, int dinodelta) {
    if (*pisJumping) {
        *pdinoy -= dinodelta;
    }
    else {
...
// 공룡의 움직임을 수정하는 함수
void ModifyDinoMove(bool* pisJumping, bool* pisBottom, int dinoy) {
    if (dinoy <= 1) {
        *pisJumping = false;
    }

    if (dinoy >= DINOSAUR_BOTTOM_Y) {
