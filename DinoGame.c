...
// Delta 구조체 생성자
Delta* CreateDelta(int dinov, int obstacleh) {
    Delta* delta = malloc(sizeof(Delta));
    if (delta != 0) {
        delta->dinov = dinov;
        delta->obstacleh = obstacleh;
        return delta;
    } else {
        return NULL;
    }
...
int DinoDelta(int* p_obstaclecase, Delta* d1, Delta* d2) {
    switch (*p_obstaclecase) {
    case 1:
        return d1->dinov;
        break;
    case 2:
...
int main(void) {
    ...
    Delta* Tree = CreateDelta(5, 10);
    Delta* Pyramid = CreateDelta(4, 6);
    while (true) {
